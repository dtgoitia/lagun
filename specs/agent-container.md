...
- Claude Code's config/session state persists via project-local convention: `.agent/config` → `/root/.config`, `.agent/claude` → `/root/.claude`. The agent container runs as **root** at **`/workspace`**. Baking in a real unprivileged `lagun` user + home dir is out of scope (deferred to `specs/varlock.md`).
